  ...
  const renderSchools = () => (
    <div>
      {items.map((school, schoolIndex) => (
        <div key={school.id} className="mb-10 fade-in" style={{ animationDelay: `${schoolIndex * 0.1}s` }}>
          <div className="flex items-center mb-4">
            <h3 className="text-xl font-bold text-[#00262D] slide-in">{school.name}</h3>
            <button
  ...
  return (
    <section className="py-8 md:py-12 fade-in">
      <div className="max-w-[1200px] mx-auto px-4">
        <h2 className="text-2xl md:text-3xl font-bold text-[#00262D] mb-4 slide-in">{title}</h2>
        {subtitle && <h3 className="text-xl font-medium text-[#00262D] mb-8 slide-in" style={{ animationDelay: '0.1s' }}>{subtitle}</h3>}
        ...
        {activeFilters.length > 0 && (
          <div className="mt-6 flex items-center slide-in">
            ...
            <div className="flex flex-wrap gap-2">
              {type === 'subjects' && activeFilters.map((id, index) => {
